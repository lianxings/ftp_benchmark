...
int ftplib::readline(char *buf,int max,ftphandle *ctl)
{
	...
	do 	{
		...
		if ( x == -1) 		{
		    errCode = errno;
		    fprintf(stderr, "Thread:%-8ld  sock:%-4d   recv failed:%s\n", syscall(SYS_gettid), ctl->handle, strerror(errno));
			retval = -1;
			break;
		}
...
#endif	
			if (w != FTPLIB_BUFSIZ) 			{
				fprintf(stderr, "write(2) returned %d, errno = %d\n", w, errno);
				return(-1);
			}
			nb = 0;
		}
...
#endif
		if (w != nb) 		{
			fprintf(stderr, "write(3) returned %d, errno = %d\n", w, errno);
			return(-1);
		}
	}
	return len;
...
int ftplib::readresp(char c, ftphandle *nControl)
{
	...
	{
	    std::cerr << "Control socket "<< nControl->handle << " read failed: " << nControl->response << std::endl;
		return 0;
	}
	...
	if (nControl->response[3] == '-')
	{
		...
		do
		{
			if (readline(nControl->response,256,nControl) == -1)
			{
                std::cerr << "Control socket "<< nControl->handle << " read failed: " 
                          << nControl->response << ", " << match << std::endl;
				return 0;
			}
...
int ftplib::Connect(const char *host)
{
    ...
    {
        int flag = 1;
        ret = setsockopt(sControl, IPPROTO_TCP, TCP_NODELAY, (char *)&flag, sizeof(flag));
    }   
    
	if (connect(sControl, (struct sockaddr *)&sin, sizeof(sin)) == -1)  {
	    errorCode = errno;
		std::cerr << "connect to " << host << " failed: " << strerror(errorCode) << std::endl;
		net_close(sControl);
		return 0;
	}
...
int ftplib::FtpSendCmd(const char *cmd, char expresp, ftphandle *nControl)
{
    ...
    if(gFtpDebug)
        printf("Thread:%-8ld  sock:%-4d  %s\n", syscall(SYS_gettid), nControl->handle, cmd);
	...
	int ret = readresp(expresp, nControl);

    if(gFtpDebug)
        printf("Thread:%-8ld  sock:%-4d  %s\n", syscall(SYS_gettid), nControl->handle, nControl->response);
...
int ftplib::FtpOpenPasv(ftphandle *nControl, ftphandle **nData, transfermode mode, int dir, char *cmd)
{
    ...
    if(gFtpDebug)
        printf("Thread:%-8ld  sock:%-4d  %s", syscall(SYS_gettid), nControl->handle, cmd);

	if (ret <= 0) 	{
	    errorCode = errno;
        printf("Thread:%-8ld  sock:%-4d  %s  send failed:%s\n", syscall(SYS_gettid), nControl->handle, cmd, strerror(errno));
		return -1;
	}
    ...
    while (true) {
        ...
        if (connect(sData, &sin.sa, sizeof(sin.sa)) == -1)  {
            if (retry >= 5) {
                errorCode = errno;
                std::cerr << "connect to establish data connnection failed: " << strerror(errorCode) << std::endl;
                net_close(sData);
                return -1;
            }
        } else {
...
int ftplib::FtpXfer(const char *localfile, const char *path, ftphandle *nControl, accesstype type, transfermode mode)
{
    ...
    if(gFtpDebug)
        printf("Thread:%-8ld  sock:%-4d  transfer size:%lld\n", syscall(SYS_gettid), nControl->handle, dataSize);
...
void ftplib::ClearHandle()
{
    ...
    //mp_ftphandle->cmode = ftplib::port;
	mp_ftphandle->idlecb = NULL;
	mp_ftphandle->idletime.tv_sec = 60;
    mp_ftphandle->idletime.tv_usec = 0;
